...
BOT_USERNAME = "BidforWorkBot"  # 🔥 Apne bot ka username (bin @ ke) daalo, jaise "MyFreelanceBot"

LEAD_FEE = 10
INITIAL_BALANCE = 100  # Free balance (ab sirf referral aur admin add se aayega)
PRICE_DROP_INTERVAL = 180
BOOKING_EXPIRE_HOURS = 24
FREE_LEADS_LIMIT = 5  # 🔥 Har user ko 5 free leads
REFERRAL_BONUS = 2  # 🔥 Referral par ₹2
...
# Users Table (New Columns: free_leads, referred_by, referral_count, referral_earnings)
cursor.execute("""
...
# 🔥 Backward Compatibility (Purane DB mein naye columns add karne ke liye)
try:
...
def get_user(telegram_id):
    ...
    if not user:
        ...
        # New user gets 5 free leads by default
        cursor.execute("""
        ...
        cursor.execute("SELECT * FROM users WHERE telegram_id = ?", (telegram_id,))
        user = cursor.fetchone()
    # User tuple: 0:id, 1:telegram_id, 2:username, 3:wallet_balance, 4:is_banned, 
    # 5:total_earnings, 6:joined_at, 7:free_leads_remaining, 8:referred_by, 9:referral_count, 10:referral_earnings
    return user
...
# ========================
# 👋 5. BOT COMMANDS (START, TERMS, HELP, WALLET)
# ========================
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔥 REFERRAL LOGIC: Check if user came via /start ref_XXXX
    if context.args:
        payload = context.args[0]
        if payload.startswith("ref_"):
            try:
                referrer_id = int(payload.split("_")[1])
                # Check if referrer exists, is not banned, and user hasn't been referred already
                if referrer_id != user[1] and user[8] is None:
                    ref_user = get_user(referrer_id)
                    if ref_user[4] != 1:  # Referrer not banned
    ...
    # Refresh user data after referral update
    user = get_user(update.effective_user.id)
...
async def sell_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔥 Check if user has enough balance OR free leads left
...
# ========================
# 🤝 10. HIRE LOGIC (WITH FREE LEADS CHECK)
# ========================
async def hire(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔥 Check if seller can receive leads (Balance OR Free Leads)
...
async def receive_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔥 LEAD FEE DEDUCTION LOGIC: Check Free Leads First
    free_leads = seller[7]  # Index 7 is free_leads_remaining
    fee_deducted = 0
    free_used = False
    ...
    if free_leads > 0:
        ...
        # Deduct ₹10
        cursor.execute("UPDATE users SET wallet_balance = wallet_balance - ? WHERE telegram_id = ?", (LEAD_FEE, seller_id))
        fee_deducted = LEAD_FEE
        free_used = False
        lead_cost_msg = f"💰 ₹{LEAD_FEE} deducted from wallet."
    
    conn.commit()
    
    # Refresh seller data for message
    seller = get_user(seller_id)
...
def main():
    ...
    app.add_handler(CommandHandler("referral", referral_command))  # 🔥 Naya Referral Command
